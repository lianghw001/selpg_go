...
func checkCommand() {
	...
	if pageType && pageLen != 72 {
		fmt.Println("warning:When use '-f' option,'-l' will be ignored")
	}
	if flag.NArg() > 1 {
		fmt.Println("warning:Only the first argument will work, others will be ignored")
	}
...
func writeOutput(outputPage chan []byte) {
	fmt.Println("tt0", destination, "dfsa")
	if destination == "" {
